...
def _resolver(mapping):
    ...
    def resolve(metric, **_kw):
        ...
        except KeyError:  # mimic an un-fetched LFS object
            raise FileNotFoundError(f"LFS object cache not found for {metric}")
    return resolve


def test_get_telemetry(tmp_path, monkeypatch):
    import pandas as pd
    pw = tmp_path/"POWER_USAGE.csv"; tp = tmp_path/"GPU_TEMP.csv"
    ...
    monkeypatch.setattr(tools, "resolve_metric_csv",
                        _resolver({"POWER_USAGE": str(pw), "GPU_TEMP": str(tp)}))
