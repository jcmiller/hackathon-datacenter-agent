...
def test_precompute_schema_label_and_window(tmp_path):
    ...
    pd.DataFrame([
        {"job_id": "j1", "type": "train", "node_num": 1, "gpu_num": 8, "cpu_num": 16,
         "duration": 90, "queue": 5, "mem_per_pod_GB": 80, "state": "FAILED",
         "start_time": "2023-08-20 10:00:00+00:00", "end_time": "2023-08-20 10:05:00+00:00",
    ...
    assert {"job_id", "gpu_num", "power_mean", "power_max", "power_std",
            "temp_mean", "util_mean", "state", "fail_time"} <= set(df.columns)
