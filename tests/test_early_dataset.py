...
def test_build_dataset_schema_labels_and_no_future_leakage(tmp_path):
    ...
    required = {"gpu", "node", "gpu_idx", "t_ref", "event_source", "horizon_s",
                "lookback_s", "label", "GPU_TEMP_mean", "GPU_TEMP_coverage",
                "GPU_TEMP_present"}
...
def test_build_dataset_reads_through_lfs_cache_when_worktree_deleted(tmp_path):
    ...
    def _commit_lfs(name, content):
        ...
        subprocess.run(["git", "-C", str(repo), "add", f"data/utilization/kalos/{name}"], check=True, capture_output=True)
    ...
    xid = "\n".join([
        "Time,n1-0,n1-1",
        f"{_iso(_t(0))},0,0", f"{_iso(_t(1))},0,0", f"{_iso(_t(2))},0,0",
        f"{_iso(_t(3))},0,0", f"{_iso(_t(4))},0,0", f"{_iso(_t(5))},0,0",
        f"{_iso(_t(6))},43,0", f"{_iso(_t(7))},43,0",
    ]) + "\n"
    temp = "\n".join([
        "Time,n1-0,n1-1",
        f"{_iso(_t(0))},40,40", f"{_iso(_t(1))},42,40", f"{_iso(_t(2))},44,40",
        f"{_iso(_t(3))},46,40", f"{_iso(_t(4))},48,40", f"{_iso(_t(5))},80,40",
        f"{_iso(_t(6))},95,40", f"{_iso(_t(7))},95,40",
    ]) + "\n"
    ...
    result = subprocess.run(
        [sys.executable, "scripts/build_early_dataset.py",
         "--repo-dir", str(repo),
         "--metrics", "GPU_TEMP",
         "--horizons", "30", "--lookback", "45", "--neg-offset", "60",
         "--sample-period", "15",
         "--out", str(out)],
