...
def _resolve_sources(repo_dir: str, metrics: list[str]) -> dict[str, str]:
    ...
    return {name: resolve_data_path(repo_dir, rel) for name, rel in zip(names, rels)}
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--repo-dir", required=True,
                   help="acme-util repo root (e.g. data/acme-util)")
    p.add_argument("--metrics", nargs="+", default=list(DEFAULT_FEATURE_METRICS),
                   help="feature metric names under data/utilization/kalos/")
    p.add_argument("--horizons", nargs="+", type=float, default=[60, 300, 600],
                   help="prediction horizons in seconds")
    p.add_argument("--lookback", type=float, default=600,
                   help="feature lookback window in seconds before t_ref")
    p.add_argument("--neg-offset", type=float, default=3600,
                   help="same-GPU pre-event negative offset in seconds")
    p.add_argument("--sample-period", type=float, default=15,
                   help="nominal telemetry sample period (s) for coverage")
    p.add_argument("--control-gpus", nargs="*", default=None,
